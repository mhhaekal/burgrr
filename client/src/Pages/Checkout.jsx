            ...
            return (
              <div key={index}>
                <CartCard
                  ...
                  button={<RoundButton text="+" onClick={() => handlePlusQty(value.product_id)} />}
                  button1={
                    <RoundButton text="-" onClick={() => handleMinusQty(value.product_id)} />
                  }
        ...
        <div className="mt-5 text-xl font-semibold">
          ...
          <div className="flex pt-5 gap-5">
            <div className="">
              <select
                ref={payment}
                className="select select-bordered w-full max-w-xs bg-green-600 text-white hover:text-white"
              >
            ...
            <div className="">
              <Button onClick={() => onConfirm()} text="Confirm" style="btn" />{" "}
            </div>
          </div>
        </div>
      </div>
      <div className="bg-green-600 w-[40%]">
        <div className="flex justify-end m-5 p-5 text-4xl font-black text-white">
          Transaction Summary
        </div>
        <div className="bg-green-900 text-white m-5 p-5 flex flex-col gap-5">
          <div className="flex justify-between">
            <div>Subtotal</div>
            <div>Rp. {totalPay.toLocaleString()}</div>
          </div>
          <div className="flex justify-between">
            <div>Tax 10%</div>
          ...
          <div className="flex justify-between">
            ...
            <div className="font-bold text-xl">
              Rp. {(total + (total * 10) / 100).toLocaleString()}
            </div>
          </div>
        </div>
        {paymentMethod === "cash" ? (
        ...
        <div className="flex flex-col justify-center items-center gap-5">
          <Button
            style="btn hover:bg-white bg-white text-green-600 flex justify-center rounded-xl text-3xl font-extrabold w-[500px] h-[50px] flex items-center"
            text="Process Payment"
          />
          <Link to={"/cashier"}>
            <Button
              style="btn hover:bg-black hover:border-black bg-black text-white flex justify-center rounded-xl text-3xl border-black font-extrabold w-[500px] h-[50px] flex items-center"
              text="Cancel"
            />
          </Link>
        </div>
